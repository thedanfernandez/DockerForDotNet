...
if [ "$dockerRemoveConflictingContainers" == "True" ]; then
...
if [ "$dockerBuildOnly" == "False" ]; then
    ...
    if [ $dockerPublishHostPort ] && [ $dockerPublishContainerPort ]; then
        ...
        if [ "$appType" == "Web" ]; then envVars=' -e "server.urls=http://localhost:$dockerPublishContainerPort"'
    fi
    ...
    containerId=$(docker "$dockerAuthOptions" -H "$dockerServerUrl" run -t -d$publishPort$envVars $dockerRunOptions "$dockerImageName")

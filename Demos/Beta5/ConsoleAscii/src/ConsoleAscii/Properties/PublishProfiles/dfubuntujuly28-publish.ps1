    ...
    process {
        ...
        if (!$buildOnly) {
            ...
            if ($hostPort -and $containerPort) {
                ...
                if ($appType -eq "Web") { $serverUrls = ' -e "server.urls=http://localhost:{0}"' -f $containerPort }
            }

            'Starting Docker container: {0}' -f $imageName | Write-Verbose
            $command = 'docker{0} -H {1} run -t -d{2}{3}{4} {5}' -f $authOptions, $dockerServerUrl, $publishPort, $serverUrls, $runOptions, $imageName
            $command | Print-CommandString
